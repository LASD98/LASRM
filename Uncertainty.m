function [a, Unc_a, n, Unc_n, R2] = Uncertainty(p, rb)
...
Unc_m = sig * (N/delta)^0.5;                                        % incertezza del coeff "m" (che coincide con quella di "n")
Unc_n = Unc_m;                                                      % dal passaggio sopra
Unc_q = sig * ( sum( X.^2 ) / delta )^0.5;                          % incertezza del coeff "q"

a = exp(q);                                                         % calcolo del valore "a"
a_new_up = a * exp(Unc_q);                                          % calcolo della parte superiore l'incertezza di "a"
a_new_down = a / exp(Unc_q);                                        % calcolo della parte inferiore per l'incertezza di "a"
Unc_a = ( a_new_up - a_new_down) / 2;                               % calcolo di "a"
